...
def mini_batch_matrix_mul(A, B, mini_batch_size=128):
    batch = A.shape[0]
    assert A.dim() == 3
    if B.dim() == 2:
        B = B.unsqueeze(-1)
    C = torch.zeros((batch, A.shape[1], B.shape[2]), dtype=torch.float32, device="cuda")
    print("processing matrix multiplication in batch size {}".format(mini_batch_size))
    for i in range(0, batch, mini_batch_size):
        C[i:i+mini_batch_size] = torch.bmm(A[i:i+mini_batch_size], B[i:i+mini_batch_size])
    if C.dim() == 3 and C.shape[2] == 1:
        C = C.squeeze(-1)
    return C

def reverse_rotation(R):
    r = torch.zeros((R.shape[0], 4), device='cuda')
    
    r[:, 0] = 0.5 * torch.sqrt(1 + R[:, 0, 0] + R[:, 1, 1] + R[:, 2, 2])
    r[:, 1] = 0.5 * torch.sign(R[:, 2, 1] - R[:, 1, 2]) * torch.sqrt(1 + R[:, 0, 0] - R[:, 1, 1] - R[:, 2, 2])
    r[:, 2] = 0.5 * torch.sign(R[:, 0, 2] - R[:, 2, 0]) * torch.sqrt(1 - R[:, 0, 0] + R[:, 1, 1] - R[:, 2, 2])
    r[:, 3] = 0.5 * torch.sign(R[:, 1, 0] - R[:, 0, 1]) * torch.sqrt(1 - R[:, 0, 0] - R[:, 1, 1] + R[:, 2, 2])
    return r
...
class GaussianModel:
    ...
    def training_seg_only_setup(self, training_args):
        l = [
            {'params': [self._objects_dc], 'lr': training_args.feature_lr, "name": "obj_dc"},
        ]
        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
    ...
    def duplicate_setup(self, training_args, mask3d):
        print("duplicating")
        mask3d = mask3d.bool().squeeze()
        ...
        xyz_sub = xyz_sub + torch.tensor([0, -0.45, 0.7], dtype=torch.float, device="cuda").unsqueeze(dim=0)

        self._xyz = torch.cat([self._xyz, xyz_sub])
        self._features_dc = torch.cat([self._features_dc, features_dc_sub])
        self._features_rest = torch.cat([self._features_rest, features_rest_sub])
        self._opacity = torch.cat([self._opacity, opacity_sub])
        self._scaling = torch.cat([self._scaling, scaling_sub])
        self._rotation = torch.cat([self._rotation, rotation_sub])
        self._objects_dc = torch.cat([self._objects_dc, objects_dc_sub])

    def transform_setup(self, training_args, mask3d, transform):
        rotation = torch.tensor(transform["rotation"], dtype=torch.float, device="cuda").unsqueeze(dim=0)
        scaling = torch.tensor([transform["scaling"]] * 3, dtype=torch.float, device="cuda").unsqueeze(dim=0)
        ...
        S = scaling.repeat(xyz_sub.shape[0], 1)
        R = build_rotation(rotation).repeat(xyz_sub.shape[0], 1, 1)
        SR = build_scaling_rotation(scaling, rotation).repeat(xyz_sub.shape[0], 1, 1)
        T = torch.tensor(transform["translation"], dtype=torch.float, device="cuda").repeat(xyz_sub.shape[0], 1)

        transformed_xyz_sub = mini_batch_matrix_mul(SR, xyz_sub + T)

        get_rotation_sub = self.rotation_activation(rotation_sub)
        get_scaling_sub = self.scaling_activation(scaling_sub)


        old_R = build_rotation(get_rotation_sub)
        transformed_R = mini_batch_matrix_mul(R, old_R)
        transformed_rotation = reverse_rotation(transformed_R)
        transformed_rotation = torch.nan_to_num(transformed_rotation)

        old_scaling_activated = get_scaling_sub
        transformed_scaling_activated = S * old_scaling_activated
        transformed_scaling = self.scaling_inverse_activation(transformed_scaling_activated)

        self._xyz[mask3d] = transformed_xyz_sub
        self._rotation[mask3d] = transformed_rotation
        self._scaling[mask3d] = transformed_scaling

    def color_change_setup(self, training_args, mask3d, color):
        color = torch.tensor(color, device=self._features_dc.device, dtype=self._features_dc.dtype) / 255
        ...
        feature_dc_sub = self._features_dc[mask3d]
        feature_rest_sub = self._features_rest[mask3d]

        feature_dc_sub[:, :3] = RGB2SH(color)
        feature_rest_sub = torch.zeros_like(feature_rest_sub)

        self._features_dc[mask3d] = feature_dc_sub
        # self._features_rest[mask3d] = feature_rest_sub
